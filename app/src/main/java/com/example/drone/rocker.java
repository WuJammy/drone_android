package com.example.drone;//////左邊的搖桿
 ...
 * 摇杆控件
 */
class Rocker extends View {
    private static final String TAG = "RockerView";

    private static final int DEFAULT_SIZE = 400;
    private static final float DEFAULT_ROCKER_SCALE = 0.5f;/**默認半徑為背景的1/2*/
    ...
    public float pointXDistance,pointYDistance;
    ...
    private OnGetXYDistance mOnGetXYDistance;
    ...
    public Rocker(Context context, AttributeSet attrs) {
        ...
        mAreaBackgroundPaint.setAntiAlias(true);//setAntiAlias(true)->抗鋸齒(讓圖案更清晰
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        ...
        //回调模式
        mCallBackMode = getCallBackMode(typedArray.getInt(R.styleable.RockerView_rockerCallBackMode, 0));//typedArray.getInt(int index, int defValue)-->檢索index處的屬性的整數值
        typedArray.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (0 == mRockerPosition.x || 0 == mRockerPosition.y) {
            mRockerPosition.set(mCenterPoint.x,mCenterPoint.y);/////////搖桿起始位置
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:// 移出区域
                ...
                moveRocker(mCenterPoint.x, mCenterPoint.y);/////讓油門保持當前位置
                break;
        }
        return true;
    }
    ...
    public Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        // 计算角度
        double angle =360- radian2Angle(radian);

        if (lenXY + rockerRadius <=(regionRadius)) { // 触摸位置在可活动范围内
            // 回调 返回参数
            p.x=touchPoint.x - centerPoint.x;
            p.y=touchPoint.y - centerPoint.y;
            callBack(angle, (int) lenXY,p);
            return touchPoint;
        } else { // 触摸位置在可活动范围以外
            // 计算要显示的位置
            ...
            System.out.println((int)(regionRadius));

            p.x= (int) ((regionRadius)*Math.cos(angle));
            p.y= (int) ((regionRadius)*Math.sin(angle));

            callBack(angle, (int) Math.sqrt((showPointX - centerPoint.x) * (showPointX - centerPoint.x) + (showPointY - centerPoint.y) * (showPointY - centerPoint.y)),p);
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle, float distance,Point pp) {
        Log.d("distance",distance+"");

        mOnGetXYDistance.getx(pp.x);
        mOnGetXYDistance.gety(pp.y);

        if (Math.abs(distance - lastDistance) >= (baseDistance / mDistanceLevel)) {/////Math.abs-->絕對值
     ...
     *
     */
    public void setOnGetXYDistance(OnGetXYDistance dis){
        mOnGetXYDistance = dis;
    }
    ...
    public interface OnGetXYDistance{

package com.example.drone;//////////右邊的搖桿
...
class Joystick extends View {
    private static final String TAG = "RockerView2";
    ...
    private OnGetXXYYDistance mOnGetXXYYDistance;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (0 == mRockerPosition.x || 0 == mRockerPosition.y) {
            mRockerPosition.set(mCenterPoint.x,mCenterPoint.y+mAreaRadius);/////////搖桿起始位置
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:// 移出区域
                ...
                moveRocker(mCenterPoint.x, mRockerPosition.y);/////讓油門保持當前位置
                break;
        }
        return true;
    }
    ...
    public Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            // 回调 返回参数
            q.x=touchPoint.x - centerPoint.x;
            q.y=touchPoint.y - centerPoint.y;
            callBack(angle, (int) lenXY,q);
            return touchPoint;
        } else { // 触摸位置在可活动范围以外
            // 计算要显示的位置
            ...
            callBack(angle, (int) Math.sqrt((showPointX - centerPoint.x) * (showPointX - centerPoint.x) + (showPointY - centerPoint.y) * (showPointY - centerPoint.y)),q);
    ...
    private void callBack(double angle, float distance,Point pp) {
        ...
        mOnGetXXYYDistance.getx(pp.x);
        mOnGetXXYYDistance.gety(pp.y);
     ...
     *
     */
    public void setOnGetXYDistance(OnGetXXYYDistance dis){
        mOnGetXXYYDistance = dis;
    }
    ...
    public interface OnGetXXYYDistance{

        void getx(float xxxx);

        void gety(float yyyy);
